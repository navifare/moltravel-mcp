...
server = FastMCP(
    ...
    instructions=(
        ...
        "Flights, Hotels & Cars (Skiplagged):\n"
        "- `skiplagged_sk_flights_search` — search flights with hidden-city and virtual interlining options\n"
        "- `skiplagged_sk_hotels_search` — search hotels by city and dates\n"
        "- `skiplagged_sk_hotel_details` — room-level availability and pricing\n"
        "- `skiplagged_sk_cars_search` — rental car search by location and dates\n"
        "- `skiplagged_sk_destinations_anywhere` — find cheapest destinations from a city\n"
        "- `skiplagged_sk_flex_departure_calendar` / `sk_flex_return_calendar` — flexible date pricing\n\n"
        "Ferries (Ferryhopper):\n"
        "- `ferryhopper_get_ports` — list all ferry ports\n"
        "- `ferryhopper_get_direct_connections_for_ports` — find routes from a port\n"
        "- `ferryhopper_search_trips` — search ferry trips between two ports on a date\n\n"
        "Accommodation (Trivago):\n"
        "- `trivago_trivago-search-suggestions` — get location IDs for a city/region query\n"
        "- `trivago_trivago-accommodation-search` — search hotels by location ID, dates, guests\n"
        "- `trivago_trivago-accommodation-radius-search` — search hotels near coordinates\n"
        "- First call search-suggestions to get the location ID, then use it in accommodation-search\n\n"
